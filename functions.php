...
function processCompetition($cid, $cmp) {
  global $link;
  $name = $link->escape_string($cmp);
  $date = $link->escape_string($cmp['date']);
  $organizer = $link->escape_string($cmp['organizer']);
  $homepage = $link->escape_string($cmp['homepage']);
...
function processControl($cid, $ctrl) {
  global $link;
  $id = $link->escape_string($ctrl['id']);
  $name = $link->escape_string($ctrl);
  $sqlupdate = "name='$name'";
...
function processClass($cid, $cls) {
  global $link;
  $id = $link->escape_string($cls['id']);
  $ord = $link->escape_string($cls['ord']);
  $name = $link->escape_string($cls);
  $sqlupdate = "name='$name', ord='$ord'";
  updateTable("mopClass", $cid, $id, $sqlupdate);

  if (isset($cls['radio'])) {
    $radio = $link->escape_string($cls['radio']);
...
function processOrganization($cid, $org) {
  global $link;
  $id = $link->escape_string($org['id']);
  $name = $link->escape_string($org);
  $sqlupdate = "name='$name'";
...
function processCompetitor($cid, $cmp) {
  global $link;
  $base = $cmp->base;
  $id = $link->escape_string($cmp['id']);

  $name = $link->escape_string($base);
  $org = (int)$base['org'];
  $cls = (int)$base['cls'];
  $stat = (int)$base['stat'];
  $st = (int)$base['st'];
